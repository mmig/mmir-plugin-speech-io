
export const PLUGIN_ID: string = 'speechio';

export const SPEECH_ACTIVE: string = 'speech-input-active';
export const READ_ACTIVE: string = 'speech-output-active';
export const DICTATION_CONTROL: string = 'dictation-ctrl';
export const READ_CONTROL: string = 'read-ctrl';

export const SPEECH_IO_MANAGER_ID = 'mmirf/speechioManager';
export const SPEECH_IO_ENGINE_ID = 'mmirf/speechioEngine';
export const SPEECH_IO_STATES_ID = 'mmirf/state/speechio';

export const SPEECH_IO_INPUT_ID = 'mmirf/speechioInput';
export const SPEECH_IO_INPUT_ENGINE_ID = 'mmirf/speechioInput';
export const SPEECH_IO_INPUT_STATES_ID = 'mmirf/state/speechioInput';
