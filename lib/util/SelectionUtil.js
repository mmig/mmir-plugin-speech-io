...
exports.SelectionUtil = void 0;
var PREV_FONT_SIZE = 'tsp_pfontsize'; //previous value for font-size of selection-marker target (String)
var PREV_LINE_HEIGHT = 'tsp_plineheight'; //previous value for line-height in pixels of selection-marker target (Integer)
 ...
 * @returns the marker DIV
 */
function createSelectionMarker(parentElement, isCreateBlinkStyle) {
...
var SelectionUtil = /** @class */ (function () {
    function SelectionUtil(unitUtil, caretPos) {
        this.unitUtil = unitUtil;
        this.caretPos = caretPos;
        this._debug = false;
    }
    SelectionUtil.prototype.clearSelectionMarker = function () {
     ...
     *
     */
    SelectionUtil.prototype.setSelectionMarker = function (elem, start, length, target, forceDisplay) {
        ...
        if (!forceDisplay && this._selMarkerStart && (!elem || /*length === 0 ||*/ document.activeElement === elem)) {
            if (this._debug)
                console.log('clearSelectionMarker()');
        ...
        if (target) {
            if (typeof target === 'string') {
                ...
                _selOpt._container = target.container;
                _selOpt._fieldName = target.fieldName;
                _selOpt.text = function () {
                    return this._container[this._fieldName];
                };
            }
        }
        if (this._debug)
            console.log('setSelectionMarker -> [' + start + ', ' + (start + length) + ']');
        ...
        if (offsetParent) { //HACK: insert marker into relative-parent of target-element => will be moved correctly on scrolling etc.
            if (offsetParent && offsetParent !== this._selMarkerStart.parentElement) {
                ...
                if (this._selMarkerEnd)
                    this._selMarkerEnd.parentElement.removeChild(this._selMarkerEnd); //TODO do this only if it will be displayed
                if (this._selMarkerBlock)
                    this._selMarkerBlock.parentElement.removeChild(this._selMarkerBlock); //TODO do this only if it will be displayed
                parent = offsetParent;
            }
        }
        else {
        ...
        if (lines > 1) {
            if (this._debug)
                console.debug('multiline selections: ' + lines);
        ...
        if (parent) { //<- only (re-)insert, if necessary
            ...
            if (this._selMarkerBlock)
                parent.appendChild(this._selMarkerBlock);
            if (this._selMarkerEnd)
                parent.appendChild(this._selMarkerEnd);
        }
        if (target) {
            _selOpt._container = null;
            _selOpt._fieldName = null;
            _selOpt.text = null;
        }
    };
    SelectionUtil.prototype.setSelectionColor = function (color) {
    ...
    SelectionUtil.prototype.getSelectionColor = function () {
     ...
     */
    SelectionUtil.prototype.setSelectionOptions = function (options) {
        _selOpt = options;
    };
    SelectionUtil.prototype.getSelectionOptions = function () {
        return _selOpt;
    };
    /**
     ...
     */
    SelectionUtil.prototype.resetSelectionCalc = function () {
        this.caretPos.resetStyleDiv();
    };
    SelectionUtil.prototype.setHeightFromFont = function (elem, marker) {
...
//  * @param name
//  * 			the key for retrieving the data
//  * @returns TRUE if the data is "true", otherwise FALSE.
//  */
// function getDataBool(el: HTMLElement, name: string): boolean {
// 	return el.dataset[name] === 'true';
// }
//# sourceMappingURL=SelectionUtil.js.map