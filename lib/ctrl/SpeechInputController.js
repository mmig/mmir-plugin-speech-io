...
var SpeechInputController = /** @class */ (function () {
    function SpeechInputController(mmirProvider, dictTargetHandler, ignoreMmirReady) {
        var _this = this;
    ...
    Object.defineProperty(SpeechInputController.prototype, "dicationTargets", {
        get: function () {
    ...
    Object.defineProperty(SpeechInputController.prototype, "emma", {
        get: function () {
            return this._emma;
        },
        enumerable: false,
    ...
    SpeechInputController.prototype.init = function (mmirp) {
        var _this = this;
        this._mmir = mmirp.mmir;
        this._emma = mmirp.mmir.emma;
        this._manager = mmirp.mmir.speechioManager;
        this._speechEventSubscriptions = SubscriptionUtil_1.SubscriptionUtil.subscribe(mmirp.speechEvents, [
            'dictationResult',
            // 'speechCommand',
            // 'commandAction'
            //'resetGuidedInputForCurrentControl' , 'startGuidedInput' , 'resetGuidedInput' , 'isDictAutoProceed'
        ], this);
        this.dictationResults = mmirp.speechEvents.dictationResult.pipe((0, operators_1.map)(function (dictation) { return _this._processDictationResult(dictation); }));
        //FIXME should get configuration from somewhere else?
        var stopWord = this._mmir.conf.get([consts_1.PLUGIN_ID, 'dictStopWord'], '');
        if (stopWord) {
            var cancelWord = this._mmir.conf.get([consts_1.PLUGIN_ID, 'dictAbortWord'], '');
            this.setDictationCommand(stopWord, cancelWord);
        }
        if (!this._mmir.conf.getBoolean([consts_1.PLUGIN_ID, 'disableUnstableFeedback'], false)) {
            var len = require('../lib/length.js');
            var pos = require('../lib/caretPosition.js');
            this._selectionUtil = new SelectionUtil_1.SelectionUtil(len, pos);
            ...
            var fauxId = consts_1.PLUGIN_ID + '-sel-marker-faux';
            var opt = this._selectionUtil.getSelectionOptions();
            opt.fauxId = fauxId;
     ...
     *                                 speech recognition.
     */
    SpeechInputController.prototype.dictationResult = function (asrEmmaEvent) {
        if (this._debugMsg)
            console.log('dictationResult -> ', asrEmmaEvent);
    ...
    SpeechInputController.prototype.doHandleDictationResult = function (asrEmmaEvent, inputElem) {
        ...
        if (isStopped) {
            ...
            if (isActivateCommandInput) {
                (0, HapticFeedback_1.triggerClickFeedback)(); //FIXME should we use impl. from VoiceUIController here instead?
