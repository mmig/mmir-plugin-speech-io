...
var PromptReader = /** @class */ (function () {
    function PromptReader(dlg, config, media, lang) {
        this.dlg = dlg;
        this.config = config;
        this.media = media;
        this.lang = lang;
    ...
    PromptReader.prototype.doRead = function (text) {
        ...
        if (this._ttsDefaultOptions) {
            var defOpt = Object.assign({}, this.doGetDefaultReadingOptions(), opt);
            Object.assign(opt, defOpt);
        }
        this.media.perform(this._ttsCtx, 'tts', [opt]);
    };
    PromptReader.prototype.doGetDefaultReadingOptions = function () {
        if (this._ttsDefaultOptions) {
            var lang = this.lang.getLanguage();
    ...
    PromptReader.prototype._log = function (msg) {
        if (this._isLog())
            this.media._log.debug(".PromptReader: " + msg, 1);
    };
    PromptReader.prototype._logError = function (msg, err) {
        if (this._isLogError())
            this.media._log.error(".PromptReader: " + msg, err, 1);
    };
    return PromptReader;
}());
